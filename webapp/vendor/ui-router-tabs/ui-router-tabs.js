        ...
        return {
            ...
            controller: ['$scope', '$stateParams', '$http', function($scope, $stateParams, $http) {
                ...
                $scope.go = function(tab) {
                    //hack for fetching latest execution id
                    if(!tab.disable && tab.route === 'design.process_detail.execution' && $stateParams.id && !$stateParams.executionId) {
                        $http.get('/api/executions?&sorts=-lastModifiedTime&limit=1&query=flowId%3D' + $stateParams.id).then(function(data) {
                            if (data.data.content.length > 0) {
                                $state.go(tab.route, {'id' : $stateParams.id, 'executionId' : data.data.content[0].id});
                            } else {
                                auxo.sgDialogService.alert("无最近执行历史", "提示")
                            }
                        });
                    } else {
                        if (!currentStateEqualTo(tab) && !tab.disable) {
                            tab.params.flowType = $stateParams.flowType;
