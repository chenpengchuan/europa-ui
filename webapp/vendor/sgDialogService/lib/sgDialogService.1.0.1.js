...
.constant("sgDialogService.version",[1,0,1])
//Configuration
...
.controller('sgDialogServiceConfirmController',['$scope','modalInstance',function($scope,modalInstance){
	$scope.cancel = $scope.cancelConfirm = function(){
		auxo.delHotkey($scope)
		modalInstance.closeModal(false);
	}
	$scope.ok = function () {
		auxo.delHotkey($scope)
		modalInstance.closeModal(true);
	}
	$scope.modalButtons =[
		{
			action:$scope.ok,
...
.service('sgDialogService',['$http', '$templateCache', '$controller', '$compile','$rootScope','$document','sgDialogService.config',
	function($http, $templateCache, $controller, $compile,$rootScope,$document,sgDialogServiceConfig) {
		...
		this.alert = function(text,title, callback, initialize){
		...
		var openModalInternal = function (contentTemplate, setup, modalInstance) {		    
			...
			centerTheDialog(modalInstance,setup.width);
		...
		var mousemove = function($event,modalInstance) {
				...
				var dy = modalInstance.startY + ($event.clientY - modalInstance.initialMouseY); // header/2 = 30
		...
		var centerTheDialog = function(modalInstance,width){
			var w = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
	        var h = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
			var dialogWidth = modalInstance.dialog.prop('offsetWidth');
			if(width){
				dialogWidth = width;
			}
	        var posWidth = w / 2 - dialogWidth / 2;
		    //posHeight = h / 2 - modalInstance.dialog.prop('offsetHeight') / 2;
		    var extra = dialogs.length * 10;
		    var posHeight = 17 + extra;
		    posWidth +=extra;
			...
			modalInstance.width = width;
			modalInstance.dialog.css(css);
