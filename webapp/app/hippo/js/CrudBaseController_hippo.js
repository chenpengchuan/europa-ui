function CrudBaseController_hippo(meta, $scope, $location, $window, $http, Restangular, ngDialog, $filter, $stateParams) {

	$scope.other = $scope.$parent.other;
	$scope.flowTypeFilter = "all";
	$scope.dateTypeFilter = "day";
	$scope.statsFacetsFilter='minute';
	$scope.systemTypeFilter = "all";
	$scope.alarmTypeFilter = "all";
	$scope.valueTypeFilter="mean";
	$scope.statusTypeFilter="all";
	$scope.alarmstatusTypeFilter = "unsolved";
	$scope.systemFlag=false;
	$scope.dataId="";
...
//首次进入界面是获取数据
	...
	$scope.statusColor = function(s) {return statusColorMap[s['type']];}
	...
	$scope.IngoreAlarm = function() {
    		auxo.openConfirmDialog($scope, ngDialog, "真的要忽略"+$scope.selectedRows.length+"个"+$scope.entityDisplayName+"？", function(){
    			var ids = getSelectRowIds();
    			Restangular.all($scope.restRootPath).customPOST(ids, "ingoreList").then(function(d){
    				$scope.fetchPage($scope.ptableState);
    			}, function(err){
    				auxo.openErrorDialog($scope, ngDialog, err.data.err);
    			});
...
//			width : "50%",
	...
	function getFlowTypeFilter() {
		...
		}else if($scope.alarmTypeFilter && $scope.alarmTypeFilter !== 'all' ) {
            filter += "type="+$scope.alarmTypeFilter;
        }else if($scope.statusTypeFilter && $scope.statusTypeFilter !== 'all' ) {
            filter += "status="+$scope.statusTypeFilter;
        }
        if($scope.systemTypeFilter && $scope.systemTypeFilter !== 'all'){
           filter = (filter.length>0)?filter+"&source="+$scope.systemTypeFilter:filter+"source="+$scope.systemTypeFilter
        }
        if($scope.alarmstatusTypeFilter && $scope.alarmstatusTypeFilter !== 'unsolved'){
           filter = (filter.length>0)?filter+"&status="+$scope.alarmstatusTypeFilter:filter+"status="+$scope.alarmstatusTypeFilter
        }
		return filter;
	}
	...
	$scope.onStatusTypeChange = function (statusType) {
    		$scope.other.statusTypeFilter = $scope.statusTypeFilter = statusType;
    		$scope.reloadPage();
    	}
   $scope.onAlarmTypeChange = function (alarmType) {
		$scope.other.alarmTypeFilter = $scope.alarmTypeFilter = alarmType;
		$scope.reloadPage();
	}
	 $scope.onAlarmStatusTypeChange = function (alarmStatus) {
    		$scope.other.alarmstatusTypeFilter = $scope.alarmstatusTypeFilter = alarmStatus;
    		$scope.reloadPage();
    	}
	$scope.onValueTypeChange = function (valueType) {
    		$scope.other.valueTypeFilter = $scope.valueTypeFilter = valueType;
    		$scope.reloadPage();
    	}
	$scope.onDateTypeChange = function (dateType) {
    		$scope.other.dateTypeFilter = $scope.dateTypeFilter = dateType;
    		$scope.reloadPage();
    	}
   $scope.onFiledsTypeChange = function (fileds) {
       		$scope.other.Fileds = $scope.Fileds = fileds;
       		$scope.reloadPage();
       	}
   $scope.onstatsFacetsChange = function (statsFacets) {
       		$scope.other.statsFacetsFilter = $scope.statsFacetsFilter = statsFacets;
       		$scope.reloadPage();
   	}
   $scope.changeCurrentCount = function (count) {
       		$scope.other.currentCount = $scope.currentCount = count;
       		$scope.reloadPage();
       	}
    $scope.onSystemTypeChange = function (systemType) {
          		$scope.other.systemTypeFilter = $scope.systemTypeFilter = systemType;
          		$scope.reloadPage();
          	}
   $scope.onUserChange = function (userFilter) {
   if($scope.userArray&&userFilter === $scope.userArray[0].name){
     $scope.other.userFilter = $scope.userFilter = null;
   }else{
   		$scope.other.userFilter = $scope.userFilter = userFilter;
   }
   		$scope.reloadPage();
   }
//按系统类别统计
     $scope.SystemType = function() {
       $scope.other.systemFlag = $scope.systemFlag=true;
       $scope.reloadPage();
     }
//总体趋势统计
     $scope.AllType = function() {
      $scope.other.systemFlag = $scope.systemFlag=false;
       $scope.reloadPage();
     }
//刷新页面
