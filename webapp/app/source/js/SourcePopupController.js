...
function SourceBaseController(meta, $scope, Restangular,$stateParams) {
    /**
    ...
    $scope.fetchPage = function(tableState) {
        ...
        $scope.sourceId = $scope.inputData.sourceId;
        $scope.sourceType = $scope.inputData.sourceType;
        Restangular.one($scope.restRootPath).get({sourceId : $scope.sourceId, sourceType : $scope.sourceType, query : queryWord, offset: tableState.pagination.start * $scope.limit, limit : $scope.limit, filter : filter, sorts : sorts})
//        Restangular.one("synchronizations/getSource").get({"sourceId":$stateParams.sourceId,"sourceType":$stateParams.sourceType,query : queryWord, offset: tableState.pagination.start * $scope.limit, limit : $scope.limit, filter : filter, sorts : sorts})
...
App.controller('SourcePopupController', function SourcePopupController($scope, Restangular, modalInstance) {
   ...
   */
    ...
    var meta = {
        restRootPath:"sources",
        entityDisplayName:"sources",
        getBaseFilter: function() {
            return "";
        },
        rowHeaders : [
            {name : "id", disName : "id", converter : auxo.same, visible: false},
            {name : "name", disName : "数据源名称", converter : auxo.same, visible: true},
    ...
    SourceBaseController.call(this, meta, $scope, Restangular)
