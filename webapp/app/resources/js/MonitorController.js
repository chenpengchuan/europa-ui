App.controller('MonitorMainController', function MonitorMainController($scope, $location, $window, $http, $stateParams, Restangular, ngDialog, Auth){
    ...
    $scope.selectTab = function(tab) {
        if ($scope.selectedTabStatus != tab.status) {
            $location.path("/monitorMain/summary/"+tab.status).search({currPage:1});
        }
    };
...
var MonitorController = function($filter, $scope, $location, $window, $http, $stateParams, Restangular, ngDialog) {
...
App.controller('MonitorController', MonitorController)
