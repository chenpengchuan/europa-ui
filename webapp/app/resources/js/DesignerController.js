...
function DesignerController($scope,  $location, $timeout, $stateParams, Restangular, sgDialogService, hotkeys){
...
//		$scope.flowType = DATAFLOW
	...
	$scope.stepImages = {
		//"source":"step_source.png",
		//"decision":"step_ifelse.png", //添加if—else
		//"transform": "step_transform.png",
		//"cache":"step_map.png",
		//"mapping":"step_map.png",
		//"mapping_chainmap":"step_map.png",
		//"mapping_conditionalmap":"step_map.png",
		//"aggregate":"step_aggregate.png",
		//"clean":"step_clean.png",
		//"filter":"step_filter.png",
		//"supplement":"step_supplement.png",
		//"union":"step_union.png",
		//"intersect":"step_intersect.png",
		//"minus":"step_minus.png",
		//"distinct":"step_distinct.png",
		//"top":"step_top.png",
		//"join":"step_join.png",
		//"starjoin":"step_join.png",
		//"split":"step_split.png",
		//"sink":"step_sink.png",
		//"exclusive": "step_exclusive.png",
		//"parallel": "step_parallel.png",
		//"shell": "step_shell.png",
		//"spark": "step_spark.png",
		//"dataflow": "step_dataflow.png",
		//"streamflow": "step_streamflow.png",
		//"nifi": "step_nifi.png",
		//"sql": "step_sql.png",
		//"lookup": "step_lookup.png",
		//"event":"step_event.png",
		//"hive":"step_hive.png",
		//"validate": "step_hive.png"
	...
	$scope.action = {
		read: $stateParams.id != undefined && $stateParams.action == "read",
		edit: $stateParams.id != undefined && $stateParams.action == "edit",
		copy: $stateParams.id != undefined && $stateParams.action == "copy",
		create: $stateParams.id == undefined || $stateParams.id == "new" || $stateParams.action == 'new'
	}
//	console.log("###########Action:" + JSON.stringify($scope.action));
	...
	$scope.fetchDataflow = function (id) {
		var initDataflowData = function(dataflow) {
			...
			if($scope.flowType !== dataflow.flowType) {
			...
			function  removeNotMatched( ) {
				...
				auxo.treeWalk(data, function (key, value, path, parent) {
					if(value && value.tags) {
						...
						if(wf && df && st)
							return;
						if (value.id=="supplement" || (isWorkflow() && !wf) || (isDataflow() && !df) || (isStreamflow() && !st) || value.group == "Datamining" || value.group == "Analysis" || value.id == "nifi" || value.id == "spark_show" || value.id == "spark_sqlsource" || value.id == "spark_analyzeDataset"){
				...
				if(searchParams.version) {
					Restangular.one("flows","history").customGET(id+"/"+searchParams.version, {latestName:true})
						.then(function (facetResult) {
							var newData = facetResult;
							initDataflowData(newData);
						})
				} else {
		...
		if(!cache.steps && !$scope.data.stepsInfo) {
			Restangular.one("steps/gquery").get()
				.then(function (facetResult) {
					...
					loadDataflow(id);*/
					//var json = JSON.stringify(facetResult);
					cache.steps = facetResult;
					var content = cache.steps.content;
                    var file = {name:"../step/europaStep.txt"};
					var readFile = new FileReader();


					 Restangular.one("/europa/collectors/test").get({})
                    .then(function (step) {
                        $scope.step = step;
					...
					/*var addContent = {
                                          "id" : "spark_lookupxs",
                                          //"name":　"lookup",
                                          "tags" : ["IO","dataflow"], // 分组 和 支持的flow类型
                                          "type" : "lookupxs",
                                          "inputConfigurations" : [{
                                              "id" : "string?options=input",
                                              "fields" : "fields"
                                          ...
                                          "outputConfigurations" : [{
                                              "id" : "string?options=output",  // 可能是多个
                                              "fields" : "fields"
                                          }],
                                          "otherConfigurations" : { // 目前这些属性没有使用, 用的是前端的
                                              //"numMappings" : "int",
                                              "rulefileSettings" : [{
                                                  "dataset" : "dataset",
                                                  "schema" : "schema",
                                                  "inputKeyColumn" : "string?options=${inputConfigurations.fields.name}",
                                                  "ruleValueColumn" : ["string?options=${schema.fields.name}"],
                                                  "outputColumn" : ["string?options=${ruleValueColumn}"],
                                                  "defaultValue" : ["string", ".length=${ruleValueColumn.length}"],
                                                  "nullDefaultValue" : ["string", ".length=${ruleValueColumn.length}"],
                                                  "interceptor":"string"
                                              }]
                                          }
                                      };
					content[0].content.push(addContent);*/
                    initStepsInputOutput(cache.steps);
                    initStepsData(auxo.clone(facetResult));
                    loadDataflow(id);
				})
	...
	$scope.redraw = function (refresh) {
       ...
       function fresh() {
		   ...
		   if ($scope.jplumbInstance) {
             ...
             $scope.fetchDataflow($stateParams.id);
             if($scope.data.dataflow.id == undefined) {
                 $scope.data.dataflow.name = $scope.flowType + "-" + auxo.getCurrentTime();
             }
       }
	};
	...
	$scope.$on('ngRepeatFinished', function (ngRepeatFinishedEvent) {
		...
		console.log("ngRepeatFinished europaeuropaeuropaeuropax");
	...
	$scope.isSaveDisabled = function () {
		return !$scope.data.dataflow.steps || $scope.data.dataflow.steps.length < 1;
	}

	$scope.loadFlow = function (id, version) {
		if(id)
			auxo.loadPage('/designer/dataflow/' + id + '/edit/' + $scope.flowType, {version: version},$scope);
	}
	...
	$scope.goBack = function () {
		if(isDataflowChanged()) {
			sgDialogService.confirm(auxo.buildErrorMsg("当前的修改会丢失，确定要返回吗?","question"), function (result) {
				if (result) {
					setStateChangeAlert();
					auxo.loadPage(auxo.meta.flow.currUrl);
				}
			}, "确认")
		} else {
			setStateChangeAlert()
		...
		if (dataflow.id) {
			...
			Restangular.all("flows").post(dataflowCopy)
				.then(function (facetResult) {
					...
					$scope.data.dataflow.id = facetResult.id;
					$stateParams.id = facetResult.id;
					dataflowCopy.id = facetResult.id;
					...
					auxo.loadPage('/designer/dataflow/' + facetResult.id + '/edit/' + $scope.flowType, {version:facetResult.version  });
				}, function (error) {
					sgDialogService.alert(auxo.getResponseErrorMsg(error), "错误");
				})
		}
	...
	$scope.submitDataflow = function () {
		var saveScheduleData = function (scheduleData) {
			...
			data.source = scheduleData.source;
			...
			Restangular.all("schedulers").post(data)
				.then(function (facetResult) {
					...
					$location.path('/designer/dataflow/' +data.flowId + '/edit/'+$scope.flowType);
				},function (error) {
	...
	$scope.renew = function (flowType) {
		 if(($scope.flowType == flowType || !flowType) && $scope.action.create){
			...
			var path =  '/designer/dataflow//new/';
			if(isWorkflow())
				path += WORKFLOW;
			else if(isDataflow())
				path += DATAFLOW;
